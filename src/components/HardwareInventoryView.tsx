...
export const HardwareInventoryView: React.FC<HardwareInventoryViewProps> = ({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* 4. Hardware Inventory Table — Desktop (md and up) */}
      ...
      <div className="md:hidden space-y-3">
        {filtered.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-xl py-12 text-center shadow-sm">
            <Box className="w-8 h-8 mx-auto mb-2 text-slate-300" />
            <p className="text-xs font-semibold text-slate-600">No hardware found</p>
            <p className="text-[11px] text-slate-400 mt-0.5">Try clearing filters or register a new device.</p>
          </div>
        ) : (
          filtered.map((item) => {
            const assignedMember = item.assignedMemberId ? members.find(m => m.id === item.assignedMemberId) : null;
            return (
              <div key={item.id} className="bg-white border border-slate-200 rounded-xl p-3.5 shadow-sm">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-semibold text-slate-900 text-sm truncate">{item.deviceModel}</div>
                      <div className="text-[11px] text-slate-500">{item.deviceCategory}</div>
                    </div>
                  </div>
                  <button
                    onClick={() => handleViewItemDetails(item)}
                    className="flex items-center gap-1 font-mono font-bold text-blue-800 px-2 py-1 rounded-lg border border-blue-200 bg-blue-50 text-[11px] shrink-0"
                  >
                ...
                <div className="flex flex-wrap items-center gap-1.5 mb-2.5">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                    ...
                    {item.status === 'Available' ? 'In Stock' : item.status === 'Assigned' ? 'Assigned' : 'Maintenance'}
                  </span>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                    item.condition === 'Brand New' ? 'bg-emerald-50 text-emerald-800 border-emerald-200' :
                    item.condition === 'Good' ? 'bg-blue-50 text-blue-800 border-blue-200' :
                    item.condition === 'Fair' ? 'bg-slate-100 text-slate-700 border-slate-200' :
                    'bg-rose-50 text-rose-800 border-rose-200'
                  }`}>{item.condition}</span>
                  {item.serialNumber && (
                    <span className="text-[10px] font-mono text-slate-500">S/N: {item.serialNumber}</span>
                  )}
                </div>

                {item.status === 'Assigned' && (item.assignedMemberName || assignedMember) && (
                  <div className="text-xs text-slate-600 mb-2.5 pb-2.5 border-b border-slate-100">
                    Assigned to: <span className="font-semibold text-slate-900">{item.assignedMemberName || assignedMember?.employeeName}</span>
                    {assignedMember?.jobTitle && <span className="text-slate-500"> · {assignedMember.jobTitle}</span>}
                  </div>
                )}
                ...
                <div className="flex items-center gap-1.5">
                  {item.status === 'Available' ? (
                    <button
                      onClick={() => handleOpenAssignModal(item)}
                      className="flex-1 flex items-center justify-center gap-1 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors"
                    >
                    ...
                    <button
                      onClick={() => onUnassignHardware(item.id)}
                      className="flex-1 flex items-center justify-center gap-1 py-2 text-xs font-medium text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg transition-colors"
                    >
                    ...
                    <button
                      onClick={() => onUpdateHardwareItem({ ...item, status: 'Available' })}
                      className="flex-1 flex items-center justify-center py-2 text-xs font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg transition-colors"
                    >
                      Mark Ready
                    </button>
                  )}
                  <button
                    onClick={() => handleOpenEditModal(item)}
                    className="flex items-center justify-center py-2 px-3 text-xs font-medium text-slate-600 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors"
                  >
                    <Edit2 className="w-3.5 h-3.5" />
                  </button>
                  <button
                    onClick={() => { if (confirm(`Remove ${item.assetTag} from inventory?`)) onDeleteHardwareItem(item.id); }}
                    className="flex items-center justify-center py-2 px-3 text-xs font-medium text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg transition-colors"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
