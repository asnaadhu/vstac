  ...
  return (
    <div className="space-y-5">
      ...
      <div className="sm:hidden space-y-3">
        {sortedMembers.length === 0 ? (
          ...
          sortedMembers.map((member) => {
            ...
            return (
              <div
                key={member.id}
                className={`bg-white border rounded-xl p-3.5 shadow-sm ${isSelected ? 'border-blue-300 bg-blue-50/30' : 'border-slate-200'}`}
              >
                ...
                <div className="flex items-start gap-3">
                  <input
                    ...
                    checked={isSelected}
                    onChange={() => toggleSelectOne(member.id)}
                    className="mt-1 rounded border-slate-300 bg-white text-blue-600 focus:ring-blue-500 shrink-0"
                  />
                  <button
                    onClick={() => onSelectMember(member)}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-slate-900 text-sm truncate">{member.employeeName}</span>
                      <span className={`w-2 h-2 rounded-full shrink-0 ${statusDot}`} />
                    </div>
                    <div className="text-xs text-slate-500 truncate">{member.jobTitle}</div>
                    <div className="text-xs text-slate-500 truncate">{member.email.address || 'No email'}</div>
                  </button>
                </div>

                {/* Info chips */}
                <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${statusText} bg-slate-100`}>
                    {member.status}
                  </span>
                  <span className="text-[10px] font-medium px-2 py-0.5 rounded-full text-slate-600 bg-slate-100">
                    {member.department}
                  </span>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${member.email.licenseType === 'None' ? 'text-slate-400 bg-slate-100' : 'text-sky-700 bg-sky-50'}`}>
                    {member.email.licenseType}
                  </span>
                  {primaryTag ? (
                    <button
                      onClick={() => onViewHardwareQR ? onViewHardwareQR(primaryTag) : onSelectMember(member)}
                      className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full text-blue-800 bg-blue-50 border border-blue-200 flex items-center gap-1"
                    >
                      <QrCode className="w-3 h-3 text-blue-600" />
                      {primaryTag}
                      {member.hardware.length > 1 && <span className="text-slate-400">+{member.hardware.length - 1}</span>}
                    </button>
                  ) : memberHasUntaggedHardware(member) ? (
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full text-rose-600 bg-rose-50 flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> Untagged
                    </span>
                  ) : (
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full text-slate-400 bg-slate-100">No hardware</span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1 mt-3 pt-2.5 border-t border-slate-100">
                  <button
                    onClick={() => onSelectMember(member)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-slate-700 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors"
                  >
                  ...
                  {onViewMemberQR && (
                    <button
                      onClick={() => onViewMemberQR(member)}
                      className="flex items-center justify-center py-2 px-3 text-xs font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors"
                    >
                      <QrCode className="w-3.5 h-3.5" />
                    </button>
                  )}
        ...
        <div className="px-1 py-2 text-center text-xs text-slate-500">
          Showing <strong className="text-slate-900 font-mono">{sortedMembers.length}</strong> of{' '}
          <strong className="text-slate-900 font-mono">{members.length}</strong> members
        </div>
      </div>
